...
import { textMsg } from '../lib/messages.js';
...
if (cmd === 'name') {
    ...
    if (!name) {
        console.error('Usage: quibble name <display name>');
        process.exit(1);
    }
...
if (cmd === 'create') {
    ...
    const target = args[1];
    if (!target) {
        console.error('Usage: quibble join <link|hexKey>');
        process.exit(1);
    }
...
async function handleCommand(input) {
    ...
    switch (cmd) {
        ...
        case 'add-writer': {
            ...
            if (!hexKey) {
                console.log('Usage: /add-writer <hexKey>');
                return;
            }
