...
export async function getRoomModerationState(room) {
    ...
    for (const msg of history) {
        ...
        if (msg.action === 'channel-kick' && msg.data?.targetKey && msg.data?.channelId) {
            ...
            if (!kickedByChannel.has(channelId))
                kickedByChannel.set(channelId, new Set());
...
export async function getModerationError(room, publicKeyHex, channelId = null) {
    ...
    if (moderationState.bans.has(publicKeyHex))
        return 'You are banned from this room.';
    if (moderationState.kickedFromRoom.has(publicKeyHex))
...
export async function resolveUserByUsername(room, username) {
    ...
    for (let i = history.length - 1; i >= 0; i--) {
        const msg = history[i];
        if (!msg?.sender || !msg?.senderName)
            continue;
        if (String(msg.senderName).trim().toLowerCase() !== target)
            continue;
...
export async function channelIsModOnly(room, channelId) {
    ...
    for (const msg of history) {
        if (msg?.type !== 'system' || msg?.action !== 'channel-add' || !msg?.data?.id)
            continue;
