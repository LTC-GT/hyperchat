...
function renderPinnedBar() {
    if (!state.activeRoom || !dom.pinnedBar || !dom.pinnedList)
        return;
...
function renderThreadPanel() {
    if (!state.activeRoom || !dom.threadPanel)
        return;
...
function addCallEventCard(msg, opts = {}) {
    if (!state.activeRoom || !msg?.data)
        return;
    if (!callMatchesCurrentView(msg.data, state.activeRoom))
        return;
    if (!dom.callEventFeed)
        return;
    const persist = opts.persist !== false;
    const roomKey = state.activeRoom;
    if (!state._callEventTimers)
        state._callEventTimers = new Map();
    if (!state.sessionCallEventsByRoom)
        state.sessionCallEventsByRoom = new Map();
    const eventId = String(msg._sessionEventId || `${Date.now()}-${Math.random().toString(16).slice(2)}`);
    if (!msg._sessionEventId)
        msg._sessionEventId = eventId;
    if (persist) {
    ...
    let text = '';
    if (msg.action === 'call-start')
        text = `${msg.senderName || 'Someone'} started a ${msg.data?.mode || 'voice'} call in ${getCallScopeLabel(msg.data)}`;
    if (msg.action === 'call-join')
        text = `${msg.senderName || 'Someone'} joined the call`;
    if (msg.action === 'call-end')
        text = 'Call ended';
...
function renderCallEventFeed() {
    if (!state.activeRoom || !dom.callEventFeed)
        return;
    if (!state._callEventTimers)
        state._callEventTimers = new Map();
...
function rebuildModerationState(roomKey) {
    ...
    for (const msg of msgs) {
        ...
        if (msg.action === 'channel-kick' && msg.data?.targetKey && msg.data?.channelId) {
            ...
            if (!kicks.has(channelId))
                kicks.set(channelId, new Set());
...
function isCurrentUserKickedFromChannel(roomKey, channelId) {
    ...
    if (roomKicks.has(state.identity?.publicKey))
        return true;
...
function renderFriendsHome() {
    ...
    if (dom.friendRequestCount)
        dom.friendRequestCount.textContent = String(state.friendRequests.size);
...
function updateSecurityStatus() {
    if (!dom.securityPeers || !dom.securityConn)
        return;
