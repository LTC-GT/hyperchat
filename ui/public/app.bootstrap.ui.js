...
function fitUserSettingsModal() {
    ...
    if (!modal || !scaleWrap || !panel || modal.classList.contains('hidden'))
        return;
...
function closeUserSettings() {
    ...
    if (dom.userSettingsPanel)
        dom.userSettingsPanel.style.zoom = '1';
...
function positionStatusMenu() {
    if (!dom.userStatusMenu || !dom.btnProfileQuick)
        return;
    const anchorRect = dom.btnProfileQuick.getBoundingClientRect();
    const menuWidth = 164;
    const viewportPadding = 8;
    const left = Math.max(viewportPadding, Math.min(anchorRect.left + 6, window.innerWidth - menuWidth - viewportPadding));
...
function hydrateSettingsModal() {
    ...
    if (dom.settingsEnableHD)
        dom.settingsEnableHD.checked = state.settings.enableHD !== false;
    if (dom.settingsRecordSelf)
        dom.settingsRecordSelf.checked = state.settings.recordSelfInCall !== false;
    ...
    if (dom.settingsCustomStunUrl)
        dom.settingsCustomStunUrl.value = state.settings.customStunUrl || '';
...
function getP2PNetworkStatusText() {
    ...
    if (state.p2pNetworkTest.status === 'testing')
        return '🟡 Checking your connection for direct peer-to-peer paths…';
    if (state.p2pNetworkTest.status === 'friendly')
        return `🟢 Good news: direct P2P should work (${label || 'public route found'})`;
    if (state.p2pNetworkTest.status === 'unfriendly')
        return `🔴 Direct P2P may be limited (${label || 'only local ICE routes found'})`;
    if (state.p2pNetworkTest.status === 'error')
        return `🔴 We could not complete the network check (${label || 'WebRTC is unavailable'})`;
...
function renderP2PNetworkStatus() {
    if (!dom.settingsP2PStatus)
        return;
    dom.settingsP2PStatus.textContent = getP2PNetworkStatusText();
    if (dom.settingsP2PExplain)
        dom.settingsP2PExplain.textContent = getP2PNetworkExplanationText();
    if (dom.btnRetestP2P) {
...
function summarizeCandidateTypes(typesSet) {
    ...
    if (types.length === 0)
        return 'no ICE candidates gathered';
...
async function runP2PNetworkTest() {
    ...
    if (token !== state.p2pNetworkTest.runToken)
        return;
...
dom.btnProfileQuick?.addEventListener('click', (event) => {
    if (event.target?.closest('#btnToggleMicGlobal') || event.target?.closest('#btnToggleCameraGlobal') || event.target?.closest('#btnUserSettingsSidebar') || event.target?.closest('#btnInvite'))
        return;
...
for (const option of document.querySelectorAll('.status-option')) {
    option.addEventListener('click', () => {
        const value = option.getAttribute('data-status');
        if (!value || typeof setPresenceStatus !== 'function')
            return;
...
document.addEventListener('click', (event) => {
    if (!dom.userStatusMenu || dom.userStatusMenu.classList.contains('hidden'))
        return;
    if (event.target?.closest('#btnProfileQuick') || event.target?.closest('#userStatusMenu'))
        return;
    dom.userStatusMenu.classList.add('hidden');
});
...
dom.btnToggleMicGlobal?.addEventListener('click', (event) => {
    event.stopPropagation();
    if (typeof toggleGlobalMicrophone === 'function')
        toggleGlobalMicrophone();
});
dom.btnToggleCameraGlobal?.addEventListener('click', (event) => {
    event.stopPropagation();
    if (typeof toggleGlobalCamera === 'function')
        toggleGlobalCamera();
});
...
dom.seedPhraseUploadInput?.addEventListener('change', async (event) => {
    ...
    try {
        ...
        if (dom.seedPhraseUploadInput)
            dom.seedPhraseUploadInput.value = '';
    }
});
dom.btnResetLocalDB?.addEventListener('click', async () => {
    if (localDbResetPending)
        return;
...
function renderCreateRoomDraft() {
    if (dom.createRoomNameInput)
        dom.createRoomNameInput.value = state.createRoomDraft.name || '';
...
function requestRoomHistory(roomKey, { count = 100, beforeSeq = null } = {}) {
    if (!roomKey)
        return;
    if (state.historyLoadingByRoom.get(roomKey))
        return;
