...
function startRoomDiscoveryWindow() {
    ...
    setTimeout(() => {
        if (token !== state.boot.sessionToken)
            return;
...
function handleServerMessage(msg) {
    switch (msg.type) {
        ...
        case 'room-deleted':
            ...
            break;
        case 'history': {
            if (!state.messagesByRoom.has(msg.roomKey))
                state.messagesByRoom.set(msg.roomKey, []);
            if (!state.seenSeqByRoom.has(msg.roomKey))
                state.seenSeqByRoom.set(msg.roomKey, new Set());
            const roomMsgs = state.messagesByRoom.get(msg.roomKey);
            const seenSeq = state.seenSeqByRoom.get(msg.roomKey);
            for (const m of msg.messages) {
                if (Number.isInteger(m?._seq) && seenSeq.has(m._seq))
                    continue;
                if (!m?.id || state.seenIds.has(m.id))
                    continue;
                state.seenIds.add(m.id);
                if (Number.isInteger(m?._seq))
                    seenSeq.add(m._seq);
                roomMsgs.push(m);
            }
        ...
        case 'message': {
            ...
            const roomMsgs = state.messagesByRoom.get(msg.roomKey);
            const seenSeq = state.seenSeqByRoom.get(msg.roomKey);
            if (Number.isInteger(msg.msg?._seq) && seenSeq.has(msg.msg._seq))
                break;
            const hasId = Boolean(msg.msg?.id);
            if (hasId && state.seenIds.has(msg.msg.id))
                break;
            if (hasId)
                state.seenIds.add(msg.msg.id);
            if (Number.isInteger(msg.msg?._seq))
                seenSeq.add(msg.msg._seq);
            ...
            if (msg.msg?.type === 'system') {
                ...
                if (action === 'channel-add')
                    rebuildChannels(msg.roomKey);
                if (action === 'room-admin-set')
                    rebuildRoomAdmins(msg.roomKey);
                if (action === 'room-owner-set')
                    rebuildRoomOwner(msg.roomKey);
                if (action === 'room-name-set')
                    rebuildRoomName(msg.roomKey);
                if (action === 'room-profile-set')
                    rebuildRoomProfile(msg.roomKey);
                if (action === 'custom-emoji-add' || action === 'custom-emoji-remove')
                    rebuildRoomEmojiMap(msg.roomKey);
                if (action === 'message-pin' || action === 'message-unpin')
                    rebuildPinnedMap(msg.roomKey);
                if (action === 'message-edit')
                    applyMessageEdits(msg.roomKey);
                if (action === 'message-reaction')
                    rebuildMessageReactions(msg.roomKey);
                if (action === 'friend-request' || action === 'friend-accept')
                    rebuildFriends(msg.roomKey);
                if (action === 'room-ban' || action === 'room-unban' || action === 'room-kick' || action === 'room-unkick' || action === 'channel-kick' || action === 'channel-unkick')
                    rebuildModerationState(msg.roomKey);
                if (action === 'presence-set' && state.activeRoom === msg.roomKey)
                    updateMemberList();
                ...
                if (action === 'call-start')
                    onIncomingCallStart(msg.msg, msg.roomKey);
                if (action === 'call-join')
                    onIncomingCallJoin(msg.msg, msg.roomKey);
                if (action === 'call-signal')
                    onIncomingCallSignal(msg.msg, msg.roomKey);
                if (action === 'call-end')
                    onIncomingCallEnd(msg.msg, msg.roomKey);
                if (action === 'call-start' || action === 'call-end' || action === 'call-join')
                    addCallEventCard(msg.msg);
            }
            ...
            if (state.activeRoom === msg.roomKey) {
                ...
                renderThreadPanel();
                renderFriendsHome();
                if ((msg.msg?.type === 'system' && msg.msg.action === 'message-edit') || isReactionUpdate)
                    renderMessages();
                else
...
function applyMessageEdits(roomKey) {
    ...
    for (const entry of roomMsgs) {
        ...
        if (entry?.type !== 'system' || entry?.action !== 'message-edit' || !entry?.data?.messageId)
            continue;
...
function rebuildMessageReactions(roomKey) {
    ...
    const setReactionState = (messageId, emoji, sender, on) => {
        ...
        if (on)
            ...
            senders.delete(senderKey);
        if (senders.size === 0)
            byEmoji.delete(emojiKey);
        if (byEmoji.size === 0)
            byMessage.delete(messageKey);
    };
    for (const entry of roomMsgs) {
        ...
        if (entry?.type !== 'system' || entry?.action !== 'message-reaction')
            continue;
        if (!entry?.data?.messageId || !entry?.data?.emoji || !entry?.sender)
            continue;
...
function hydrateSetupModal() {
    if (!dom.setupFullName || !dom.setupUsername)
        return;
    if (!state.profile.username)
        state.profile.username = generateAnonUsername();
    if (!state.profile.fullName)
        state.profile.fullName = 'Anonymous';
...
dom.btnSeedBackupCopy?.addEventListener('click', async () => {
    ...
    try {
        ...
        setTimeout(() => {
            if (dom.btnSeedBackupCopy)
                dom.btnSeedBackupCopy.textContent = 'Copy Seed Phrase';
        }, 1200);
    }
    catch {
...
function collectRoomTakenUsernames(roomKey) {
    ...
    for (const msg of msgs) {
        if (!msg?.senderName)
            continue;
        if (msg.sender === state.identity?.publicKey)
            continue;
...
function buildUsernameSuggestions(baseUsername, taken) {
    ...
    const add = (candidate) => {
        ...
        if (!clean || taken.has(clean) || out.includes(clean))
            return;
        out.push(clean);
    };
...
function ensureUsernameUniquenessForRoom(roomKey) {
    if (!roomKey || roomKey !== state.activeRoom)
        return;
