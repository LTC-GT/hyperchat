...
function rebuildRoomProfile(roomKey) {
    ...
    let latest = null;
    for (const msg of msgs) {
        if (msg?.type !== 'system' || msg?.action !== 'room-profile-set' || !msg?.data)
            continue;
        latest = msg.data;
    }
    ...
    room.iconImage = typeof latest.imageData === 'string' && latest.imageData.startsWith('data:image/')
        ...
        : null;
    if (!room.iconEmoji)
        room.iconEmoji = pickDefaultRoomEmoji(roomKey);
    state.rooms.set(roomKey, room);
    renderServerList();
}
function rebuildRoomName(roomKey) {
    ...
    let latestName = '';
    for (const msg of msgs) {
        if (msg?.type !== 'system' || msg?.action !== 'room-name-set' || !msg?.data?.name)
            continue;
...
function isCurrentUserOwner() {
    if (!state.activeRoom || !state.identity?.publicKey)
        return false;
...
function updateHeaderActionVisibility() {
    ...
    if (!showMembersToggle)
        updateMembersToggleButton?.(false);
    else
    ...
    if (!showSecurityStatus)
        dom.securityTooltip?.classList.add('hidden');
    ...
    if (typeof refreshCallControlsMenu === 'function')
        refreshCallControlsMenu();
}
function hideSearchDropdown() {
    if (!dom.channelSearchDropdown)
        return;
...
function showSearchResultsView({ title, subtitle, rows }) {
    if (!dom.searchResultsView || !dom.messagesScroll)
        return;
...
function renderServerSearchDropdown(query) {
    ...
    for (const msg of roomMsgs) {
        if (msg?.type !== 'text' || msg?.dmKey || msg?.threadRootId)
            continue;
...
function resolveMemberNameByKey(roomKey, publicKey) {
    ...
    for (let i = msgs.length - 1; i >= 0; i--) {
        const m = msgs[i];
        if (m?.sender === publicKey && m?.senderName)
            return m.senderName;
    }
    return publicKey.slice(0, 12);
}
function renderAdminPanel() {
    if (!state.activeRoom || !dom.adminEmojiList || !dom.adminList)
        return;
    ...
    const ownerLabel = ownerKey
        ...
        : 'Unknown';
    if (dom.ownerCurrent)
        dom.ownerCurrent.textContent = ownerLabel;
    if (dom.ownerTransferInput) {
        dom.ownerTransferInput.disabled = !isOwner;
        if (!isOwner)
            dom.ownerTransferInput.value = '';
    }
    if (dom.btnTransferOwner)
        dom.btnTransferOwner.disabled = !isOwner;
    if (dom.btnDisbandGroup)
        dom.btnDisbandGroup.disabled = !isOwner;
    if (dom.btnDeleteServer)
        dom.btnDeleteServer.disabled = !isOwner;
    ...
    if (dom.btnAdminSetServerName)
        dom.btnAdminSetServerName.disabled = !isCurrentUserAdmin();
    ...
    if (dom.btnAdminSetServerAvatar)
        dom.btnAdminSetServerAvatar.disabled = !isCurrentUserAdmin();
    if (dom.btnAdminClearServerAvatar)
        dom.btnAdminClearServerAvatar.disabled = !isCurrentUserAdmin();
...
function renderModerationPanel() {
    if (!state.activeRoom || !dom.moderationChannelSelect || !dom.adminBanList)
        return;
    ...
    if (options.some((o) => o.value === current))
        dom.moderationChannelSelect.value = current;
...
dom.customEmojiInput.addEventListener('change', async (e) => {
    ...
    if (!file || !state.activeRoom || !isCurrentUserAdmin())
        return;
...
dom.btnAdminSetServerAvatar?.addEventListener('click', () => {
    if (!state.activeRoom || !isCurrentUserAdmin())
        return;
...
dom.serverAvatarInput?.addEventListener('change', async (event) => {
    ...
    if (dom.serverAvatarInput)
        dom.serverAvatarInput.value = '';
});
...
dom.btnTransferOwner?.addEventListener('click', () => {
    if (!state.activeRoom || !isCurrentUserOwner())
        return;
    const target = String(dom.ownerTransferInput?.value || '').trim();
    if (!target)
        return;
    if (target === state.roomOwnerByRoom.get(state.activeRoom))
        return;
...
function rebuildChannels(roomKey) {
    ...
    const msgs = state.messagesByRoom.get(roomKey) || [];
    for (const msg of msgs) {
        if (msg?.type !== 'system' || msg?.action !== 'channel-add' || !msg?.data)
            continue;
...
function updateComposerAccess() {
    ...
    if (blockedByReadOnly)
        dom.messageInput.placeholder = 'Read-only room (writer access required)';
    else if (blockedByBan)
        dom.messageInput.placeholder = 'You are banned from this room';
    else if (blockedByServerKick)
        dom.messageInput.placeholder = 'You were kicked from this server';
    else if (blockedByKick)
        dom.messageInput.placeholder = 'You were kicked from this channel';
    else if (blockedByRole)
        dom.messageInput.placeholder = 'Moderator/Admin only channel';
    else
