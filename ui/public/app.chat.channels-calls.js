...
function getVoiceChannelParticipants(channelId) {
    if (!state.activeRoom || !channelId || !state.activeCall)
        return [];
    if (state.activeCall.roomKey !== state.activeRoom || state.activeCall.channelId !== channelId)
        return [];
    ...
    for (const msg of roomMsgs) {
        if (msg?.type !== 'system')
            continue;
        if (msg.action !== 'call-start' && msg.action !== 'call-join')
            continue;
        if (msg.data?.channelId !== channelId)
            continue;
        if (msg.data?.callId !== state.activeCall.id)
            continue;
        if (!msg.sender)
            continue;
...
function renderChannelLists() {
    ...
    for (const channel of channels.voice) {
        ...
        const isConnectedHere = Boolean(state.activeCall &&
    ...
    if (!dom.adminModal?.classList.contains('hidden'))
...
function getVisibleMessagesForActiveTextChannel() {
    ...
    return roomMsgs.filter((msg) => {
        if (msg?.sender && msg.type !== 'system' && isSenderBlockedForChannel(state.activeRoom, activeText, msg.sender))
            return false;
        if (!messageBelongsToTextChannel(msg, activeText))
            return false;
        if (state.activeDmKey)
            return msg.dmKey === state.activeDmKey;
        if (msg.dmKey)
            return false;
        if (msg.threadRootId)
            return false;
...
function messageBelongsToTextChannel(msg, channelId) {
    ...
    if (msg.type === 'system') {
        if (msg.action === 'channel-add' || msg.action === 'room-admin-set' || msg.action === 'room-owner-set' || msg.action === 'room-name-set' || msg.action === 'room-profile-set' || msg.action === 'room-disband' || msg.action?.startsWith('custom-emoji-'))
            return true;
        ...
        if (msg.action === 'friend-request' || msg.action === 'friend-accept')
            return false;
        if (msg.action === 'room-ban' || msg.action === 'room-unban' || msg.action === 'room-kick' || msg.action === 'room-unkick' || msg.action === 'channel-kick' || msg.action === 'channel-unkick')
            return true;
        if (msg.action?.startsWith('call-'))
            return false;
...
dom.btnCallFullscreen?.addEventListener('click', async () => {
    if (!document.fullscreenElement)
        await dom.callStage?.requestFullscreen?.();
    else
...
dom.callBitrate?.addEventListener('change', () => {
    ...
    if (typeof setCallBitrateMode === 'function')
        setCallBitrateMode(mode);
    else
        applyCallBitrate(mode);
});
...
document.addEventListener('click', (e) => {
    if (!dom.channelSearchDropdown || !dom.channelSearch)
        return;
    const target = e.target;
    if (dom.channelSearchDropdown.contains(target) || target === dom.channelSearch || target === dom.btnChannelSearchSubmit)
        return;
    hideSearchDropdown();
