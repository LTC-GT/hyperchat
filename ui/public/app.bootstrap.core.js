...
function initAppDialogs() {
    if (!dom.appDialogModal || !dom.appDialogConfirm || !dom.appDialogCancel)
        return;
    ...
    dom.appDialogModal.addEventListener('click', (event) => {
        if (event.target !== dom.appDialogModal)
            return;
        if (activeAppDialog?.mode === 'alert')
            resolveActiveAppDialog(true);
        else
            resolveActiveAppDialog(false);
    });
    document.addEventListener('keydown', (event) => {
        if (!activeAppDialog || event.key !== 'Escape')
            return;
...
function pumpAppDialogQueue() {
    if (activeAppDialog || appDialogQueue.length === 0 || !dom.appDialogModal)
        return;
    ...
    if (dom.appDialogCancel)
        dom.appDialogCancel.textContent = String(activeAppDialog.cancelText || 'Cancel');
    if (dom.appDialogConfirm)
        dom.appDialogConfirm.textContent = String(activeAppDialog.confirmText || 'OK');
...
function resolveActiveAppDialog(confirmed) {
    ...
    if (current.mode === 'confirm') {
        ...
        if (!confirmed)
            ...
            current.resolve(String(dom.appDialogInput?.value || ''));
    }
    else {
        current.resolve();
    }
...
function showSeedBackupModal(payload) {
    ...
    if (dom.seedPhraseText)
        dom.seedPhraseText.textContent = phrase;
...
function stopRingtoneLoop() {
    if (state.ringingTimer)
        clearInterval(state.ringingTimer);
