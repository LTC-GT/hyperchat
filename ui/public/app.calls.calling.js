...
function drainPendingCandidates(remotePeerId, pc) {
    ...
    for (const c of buffered) {
        try {
            ...
            console.warn('[WebRTC] drain candidate err:', e);
        }
    }
}
...
async function callPeer(remotePeerId) {
    if (!state.localCallStream) {
        console.error('[WebRTC] callPeer: no local stream');
        return;
    }
    if (remotePeerId === getLocalPeerId())
        return; // don't call self
...
function callMatchesCurrentView(data, roomKey) {
    ...
    if (!dmKey) {
        ...
        if (channelId !== activeText && channelId !== activeVoice)
            return false;
    }
    return true;
}
function callMatchesActiveCallScope(data) {
    if (!state.activeCall)
        return false;
    if (String(data?.callId || '') !== String(state.activeCall.id || ''))
        return false;
...
async function requestCallMedia(mode) {
    try {
        ...
        if (state.settings.micId)
            audio.deviceId = { exact: state.settings.micId };
        const video = {
        ...
        if (state.settings.cameraId)
            video.deviceId = { exact: state.settings.cameraId };
...
async function startCall(mode, options = {}) {
    ...
    if (typeof renderChannelLists === 'function')
        renderChannelLists();
    send({
        type: 'start-call',
...
async function joinCall(callId, mode, channelId, options = {}) {
    ...
    state.localCallStream = stream;
    if (scope.scope === 'voice')
        state.activeVoiceChannelByRoom.set(state.activeRoom, scope.channelId);
...
async function onIncomingCallStart(msg, roomKey) {
    if (!callMatchesCurrentView(msg?.data, roomKey))
        return;
...
async function onIncomingCallJoin(msg, roomKey) {
    ...
    if (!state.activeCall)
        return;
    if (!callMatchesActiveCallScope(msg?.data || {}))
        return;
    if (msg.sender === state.identity?.publicKey)
        return;
...
async function endCall(notifyRemote) {
    ...
    if (state.localCallStream) {
        for (const t of state.localCallStream.getTracks())
            t.stop();
    }
    if (state.callScreenStream) {
        for (const t of state.callScreenStream.getTracks())
            t.stop();
    }
...
function showInlineVoiceCallControls() {
    ...
    if (dom.callStatus)
        dom.callStatus.textContent = 'Voice call active';
...
function attachLocalStream(stream) {
    ...
    if (typeof bindCallVideoFullscreen === 'function')
