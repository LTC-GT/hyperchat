...
function handleReactionPickerOutsideClick(event) {
    if (!activeReactionPicker)
        return;
    if (activeReactionPicker.contains(event.target))
        return;
    closeReactionPicker();
}
...
function openReactionPicker(anchor, msg) {
    if (!anchor || !msg?.id || !state.activeRoom)
        return;
    closeReactionPicker();
...
function addRoom(keyHex, link, opts = {}) {
    ...
    if (state.rooms.has(keyHex)) {
        ...
        if (typeof opts.writable === 'boolean')
            room.writable = opts.writable;
        renderServerList();
        if (state.activeRoom === keyHex)
    ...
    if (!state.activeTextChannelByRoom.has(keyHex))
        state.activeTextChannelByRoom.set(keyHex, 'general');
    if (!state.activeVoiceChannelByRoom.has(keyHex))
        state.activeVoiceChannelByRoom.set(keyHex, 'voice-general');
    renderServerList();
}
...
function selectRoom(keyHex) {
    ...
    if (state.membersVisible)
        dom.membersSidebar.classList.remove('hidden');
    updateAdminControls();
    renderChannelLists();
    renderMessages();
    renderPinnedBar();
...
function updateRoomWelcomeBanner() {
    if (!dom.roomWelcomeTitle || !dom.roomWelcomeDesc)
        return;
    if (!state.activeRoom) {
...
function appendMessage(msg) {
    ...
    if (!messageBelongsToTextChannel(msg, activeText))
        return;
    if (state.activeDmKey && msg.dmKey !== state.activeDmKey)
        return;
    if (!state.activeDmKey && msg.dmKey)
        return;
    if (msg.threadRootId)
        return;
...
function getPreviousGroupableMessage(messages, startIndex) {
    for (let index = startIndex; index >= 0; index--) {
        ...
        if (isGroupableMessage(candidate))
            return candidate;
    }
    return null;
}
function createMessageEl(msg, lastSender, lastTime) {
    if (msg.type === 'system')
        return createSystemMessageEl(msg);
    if (msg.type === 'file')
        return createFileMessageEl(msg, lastSender, lastTime);
    if (msg.type === 'voice')
        return createVoiceMessageEl(msg);
    if (msg.type === 'text')
        return createTextMessageEl(msg, lastSender, lastTime);
    return null;
}
...
function createSystemMessageEl(msg) {
    if (msg.action === 'call-signal' || msg.action === 'call-start' || msg.action === 'call-join' || msg.action === 'call-end')
        return null;
    if (msg.action === 'message-edit')
        return null;
    if (msg.action === 'presence-set')
        return null;
    if (msg.action === 'friend-request' || msg.action === 'friend-accept')
        return null;
    const div = document.createElement('div');
    div.className = 'flex items-center gap-2 px-1 py-1 text-xs text-quibble-text-m fade-in';
    let text = '';
    if (msg.action === 'join')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> joined the room`;
    else if (msg.action === 'leave')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> left the room`;
    else if (msg.action === 'custom-emoji-add')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> added emoji :${esc(msg.data?.name || 'emoji')}:`;
    else if (msg.action === 'custom-emoji-remove')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> removed emoji :${esc(msg.data?.name || 'emoji')}:`;
    else if (msg.action === 'room-admin-set')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> updated server admins`;
    ...
    else if (msg.action === 'room-name-set')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> renamed the server to <span class="font-medium text-quibble-text">${esc(msg.data?.name || 'Untitled')}</span>`;
    else if (msg.action === 'room-profile-set')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> updated the server profile icon`;
    else if (msg.action === 'room-disband')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> disbanded the group`;
    else if (msg.action === 'message-pin')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> pinned a message`;
    else if (msg.action === 'message-unpin')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> unpinned a message`;
    else if (msg.action === 'room-kick')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> kicked <span class="font-medium text-quibble-text">${esc(msg.data?.targetName || 'a user')}</span> from the server`;
    else if (msg.action === 'room-unkick')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> restored <span class="font-medium text-quibble-text">${esc(msg.data?.targetName || 'a user')}</span> to the server`;
    else if (msg.action === 'channel-add')
        text = `<span class="font-medium text-quibble-text">${esc(msg.senderName || 'Someone')}</span> created ${esc(msg.data?.kind || 'text')} channel ${esc(msg.data?.name || '')}`;
    else
        return null;
...
dom.messagesScroll?.addEventListener('scroll', () => {
    if (!state.activeRoom)
        return;
    if (dom.messagesScroll.scrollTop > 120)
        return;
...
function getSystemEmojiList() {
    if (Array.isArray(cachedSystemEmojiList) && cachedSystemEmojiList.length > 0)
...
function renderEmojiPicker() {
    if (!dom.emojiGrid || !dom.customEmojiGrid)
        return;
