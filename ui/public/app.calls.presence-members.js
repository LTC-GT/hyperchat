...
function getPresenceMeta(statusOverride = null, lastActivityAt = null) {
    ...
    if (status === 'away')
        return { label: 'Online - Away', dotClass: 'bg-quibble-blurple', visibleOnline: true };
...
function schedulePresenceAwayTimer() {
    if (presenceAwayTimer)
        clearTimeout(presenceAwayTimer);
    if (normalizePresenceStatus(state.settings.presenceStatus) === 'away')
        return;
...
function initPresenceTracking() {
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible')
            noteLocalPresenceActivity('visibility');
    });
...
function applyLocalMediaTrackState() {
    ...
    for (const track of state.localCallStream.getAudioTracks())
...
function getCallRecordingElements() {
    ...
    const tiles = [];
    if (includeSelf && dom.localVideo?.srcObject)
        tiles.push(dom.localVideo);
    if (dom.remoteVideos) {
...
function drawRecordingFrame() {
    ...
    if (!recording.active || !recording.canvas)
        return;
...
function syncCallRecordingAudioSources() {
    const recording = state.callRecording;
    if (!recording.active || !recording.audioContext || !recording.audioDestination)
        return;
    ...
    if (recording.includeSelf && state.localCallStream) {
        ...
        if (localAudioTracks.length)
            targetStreams.set(`self:${state.localCallStream.id}`, state.localCallStream);
    }
    for (const [peer, stream] of state.remoteStreams) {
        const audioTracks = stream.getAudioTracks().filter((track) => track.readyState === 'live');
        if (audioTracks.length)
            targetStreams.set(`peer:${peer}`, stream);
    }
    for (const [key, node] of recording.audioSources.entries()) {
        if (targetStreams.has(key))
            continue;
        try {
            node.disconnect();
        }
    ...
    for (const [key, stream] of targetStreams.entries()) {
        if (recording.audioSources.has(key))
            continue;
        try {
...
async function startCallRecording() {
    if (!state.activeCall || state.callRecording.active)
        return;
    ...
    if (recorderOptions)
        recorderOptions.videoBitsPerSecond = bitrateHint;
...
function buildCameraConstraintsFromSettings() {
    ...
    if (state.settings.cameraId)
        constraints.deviceId = { exact: state.settings.cameraId };
    return constraints;
}
...
async function toggleCallScreenShare() {
    ...
    if (state.callScreenStream) {
        for (const track of state.callScreenStream.getTracks())
            track.stop();
...
function applyCallBitrate(bitrate) {
    ...
    if (dom.callBitrate)
        dom.callBitrate.value = usingAuto ? 'auto' : String(target);
    if (dom.callBitrateMenu)
        dom.callBitrateMenu.value = usingAuto ? 'auto' : String(target);
    const effectiveVideoCap = state.settings.enableHD === false ? Math.min(target, 700000) : target;
    for (const pc of state.peerConnections.values()) {
        for (const sender of pc.getSenders()) {
            if (!sender?.track?.kind || !sender?.getParameters || !sender?.setParameters)
                continue;
...
function computeAutoCallBitrate() {
    ...
    let perPeer = 1200000;
    if (networkStatus === 'friendly')
        perPeer = 2600000;
    if (networkStatus === 'unfriendly')
        perPeer = 900000;
    if (networkStatus === 'error')
        perPeer = 800000;
    if (!hdEnabled)
        perPeer = Math.min(perPeer, 700000);
    if (isScreenShare)
        perPeer = Math.max(perPeer, 2200000);
...
function refreshCallControlsMenu() {
    ...
    if (dom.callBitrateMenu && dom.callBitrate) {
        ...
        if (dom.callBitrate.value !== mode)
            dom.callBitrate.value = mode;
...
dom.btnToggleMembers.addEventListener('click', () => {
    ...
    if (state.membersVisible && state.activeRoom)
...
function updateMemberList() {
    if (!dom.memberListActive || !dom.memberListAway)
        return;
    ...
    if (state.activeRoom) {
        ...
        for (const m of msgs) {
            ...
            if (m?.sender && m.sender !== state.identity?.publicKey) {
                ...
                if (at > prev)
                    peerLastActivity.set(m.sender, at);
            }
    ...
    for (const row of activeRows)
        dom.memberListActive.appendChild(row);
    for (const row of awayRows)
        dom.memberListAway.appendChild(row);
    if (dom.onlineActiveCount)
        dom.onlineActiveCount.textContent = String(activeRows.length);
    if (dom.onlineAwayCount)
        dom.onlineAwayCount.textContent = String(awayRows.length);
...
function createMemberEl({ key, name, avatar, isOnline, isAway = false, isSelf, isOwner = false }) {
  ...
  `;
    div.querySelector('.friend-btn')?.addEventListener('click', () => {
        if (!key || !state.activeRoom || isFriend)
            return;
