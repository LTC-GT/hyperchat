...
import { getRoomOwner, getRoomAdmins, isRoomAdmin, getRoomModerationState, getModerationError, resolveUserByUsername, channelIsModOnly, findOpenPort } from './server-room-helpers.js';
...
function scheduleAppShutdown({ wipeLocal = false } = {}) {
    ...
    setTimeout(async () => {
        ...
        if (wipeLocal) {
            ...
            if (storageDir && !storageDir.startsWith(identity.dir))
                resetPaths.add(storageDir);
...
function clearActiveCallByRoomCallId(roomKey, callId) {
    ...
    for (const [key, value] of activeCallsByScope.entries()) {
        if (!key.startsWith(`${roomPart}|`))
            continue;
        if (String(value?.callId || '') !== id)
            continue;
...
function unpersistRoom(roomKey, link = null) {
    ...
    const rooms = loadPersistedRooms().filter((entry) => {
        if (key && entry.roomKey === key)
            return false;
        if (invite && entry.link === invite)
            return false;
        return true;
    });
    savePersistedRooms(rooms);
...
wss.on('connection', (ws) => {
        ...
        try {
            switch (msg.type) {
                ...
                case 'send-message': {
                    ...
                    if (msg.threadRootId)
                        chatMsg.threadRootId = String(msg.threadRootId);
                    if (msg.dmKey)
                        chatMsg.dmKey = String(msg.dmKey);
                ...
                case 'toggle-message-reaction': {
                    ...
                    if (dmKey) {
                        const participants = new Set((Array.isArray(target.dmParticipants) ? target.dmParticipants : [])
                            ...
                            .filter(Boolean));
                        if (target.sender)
                            participants.add(String(target.sender));
                ...
                case 'upload-file': {
                    const room = quibble.rooms.get(msg.roomKey);
                    if (!room || !msg.fileName || !msg.dataBase64)
                        break;
                    const channelId = msg.channelId || 'general';
                    const dmKey = msg.dmKey ? String(msg.dmKey) : null;
                    const senderHex = b4a.toString(identity.publicKey, 'hex');
                ...
                case 'kick-user-channel': {
                    const room = quibble.rooms.get(msg.roomKey);
                    if (!room || !msg.username || !msg.channelId)
                        break;
                    const requesterHex = b4a.toString(identity.publicKey, 'hex');
                    const allowed = await isRoomAdmin(room, requesterHex);
                    if (!allowed) {
                ...
                case 'add-custom-emoji': {
                    const room = quibble.rooms.get(msg.roomKey);
                    if (!room || !msg.name || !msg.imageData)
                        break;
                    const requesterHex = b4a.toString(identity.publicKey, 'hex');
                    const allowed = await isRoomAdmin(room, requesterHex);
                    if (!allowed) {
                ...
                case 'set-room-admins': {
                    const room = quibble.rooms.get(msg.roomKey);
                    if (!room || !Array.isArray(msg.admins))
                        break;
                    const requesterHex = b4a.toString(identity.publicKey, 'hex');
                    const allowed = await isRoomAdmin(room, requesterHex);
                    if (!allowed) {
    ...
    ws.on('close', () => {
        ...
        for (const [keyHex, watchers] of roomWatchers) {
            ...
            if (watchers.size === 0)
                roomWatchers.delete(keyHex);
        }
    });
});
function startWatching(room, keyHex, ws) {
    if (!roomWatchers.has(keyHex))
        roomWatchers.set(keyHex, new Set());
...
async function getLatestUserReactionState(room, { messageId, emoji, senderKey }) {
    ...
    while (true) {
        ...
        for (const candidate of page.messages) {
            if (!candidate || String(candidate.sender || '') !== senderKey)
                continue;
            let targetId = null;
            ...
            if (targetId !== messageId || reactionEmoji !== emoji)
                continue;
