...
export class Room extends EventEmitter {
  ...
  // ─── Replication (called by Neet on each swarm connection) ───
...
/**
 * Parse a pear://neet/... link into a room key buffer.
 */
export function parseLink (link) {
  if (!link.startsWith(NEET_PREFIX)) {
    throw new Error(`Invalid neet link (expected ${NEET_PREFIX}…)`)
  }
  const payload = z32.decode(link.slice(NEET_PREFIX.length))
  if (payload.length < INVITE_KEY_BYTES) {
    throw new Error('Invalid neet invite payload')
  }
...
function deriveMessageKey (roomKey) {
  ...
  const ctx = b4a.from('neet-room-msg-v1')
