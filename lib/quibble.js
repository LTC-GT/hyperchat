...
export class Quibble extends EventEmitter {
    ...
    constructor(opts) {
        ...
        this.swarm.on('connection', (socket, info) => {
            ...
            if (!this._peersByKey.has(pkHex))
                this._peersByKey.set(pkHex, new Set());
    ...
    * Join an existing room by key buffer or pear://quibble/... link.
    ...
    _attachWriterSync(socket) {
        if (this._writerSyncBySocket.has(socket))
            return;
    ...
    _announceRoomToSocket(room, socket) {
        const channel = this._writerSyncBySocket.get(socket);
        if (!channel || !room?.key || !room?.base?.local?.key)
            return;
        const payload = {
    ...
    _onWriterSyncMessage(buf) {
        ...
        if (payload?.type !== 'room-local-writer')
            return;
        const roomKey = String(payload.roomKey || '').trim().toLowerCase();
        const writerKey = String(payload.writerKey || '').trim().toLowerCase();
        if (!HEX_KEY_RE.test(roomKey) || !HEX_KEY_RE.test(writerKey))
            return;
        const room = this.rooms.get(roomKey);
        if (!room || !room.writable || !room.base?.local?.key)
            return;
        const localWriterKey = b4a.toString(room.base.local.key, 'hex');
        if (writerKey === localWriterKey)
            return;
        if (!this._writerGrantCache.has(roomKey))
            this._writerGrantCache.set(roomKey, new Set());
        const granted = this._writerGrantCache.get(roomKey);
        if (granted.has(writerKey))
            return;
