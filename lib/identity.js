...
import b4a from 'b4a';
import { generateMnemonic, validateMnemonic, entropyToMnemonic, mnemonicToEntropy, wordlists } from 'bip39';
...
async function applyIdentity(nodes, view) {
    for (const node of nodes) {
        ...
        if (!value || value.type !== 'identity-state')
            continue;
...
function readLegacyIdentity(dir) {
    ...
    try {
        ...
        if (!legacy?.secretKey || !legacy?.publicKey)
            return null;
        return {
            path: legacyPath,
            data: legacy
